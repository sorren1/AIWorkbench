...
The same typed provider contract has an explicit `--provider e2b` implementation using the pinned official E2B SDK. It uploads only the synthetic repository snapshot and approved artifacts, requests deny-all egress, requires a blocked outbound probe, records provider-specific metadata in evidence schema v2, and uses `finally` cleanup plus a two-minute provider lifecycle. Because no `E2B_API_KEY` was available for this revision, it is implemented but not live-validated and contributes no checked-in run evidence.
