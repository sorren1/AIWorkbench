...
Vercel is the selected Git-backed static host. Version 1.0.8 is deployed in Production at `https://tylerwilhite.dev`, with the project namespace at `/workbench/`. Its audited source `fc2957843077606a1cdb8fe9101cbed9421fb243` contains no release summary; direct child `1c1c06b8e5c6973604b025b63aafed606b2bd522` adds only that generated file, annotated tag `v1.0.8` points to the child, and the same child is deployed. The hosted [release summary](https://tylerwilhite.dev/security/release-summary.json) and [deployment binding](https://tylerwilhite.dev/security/deployment-binding.json) are authoritative for that relation. Preview builds still omit canonical-dependent output. Production builds accept the origin only through validated `SITE_CANONICAL_URL`, which drives HTML metadata, robots/sitemap output, and RFC 9116 canonical identity. This verifies the exact static release artifact, not Production reliability, shared identity/state, real external integrations, or live E2B/LiteLLM/provider operation. See [ADR: static hosting](docs/adr/static-hosting-and-security-headers.md) and [deployment verification](docs/deployment-verification.md).
