...
```text
Static visitor
  ├─ /                                  semantic portfolio index
  └─ /workbench/                       governed-delivery case study
...
Vite creates a multi-page static build: the portfolio index, case-study HTML, and article retain their substantive content without JavaScript; React is bundled only for the demo entry. Local development keeps the physical `/`, `/writing/`, and `/demo/` routes. Vercel maps the portfolio index to `/` and the complete project surface to `/workbench/`, with narrow legacy-page redirects rather than an SPA fallback. Build-time generation injects optional configured links and metadata, synchronized code excerpts, the latest validated evidence, `robots.txt`, `sitemap.xml`, and static-host header manifests. Assets are local and the CSP does not require `unsafe-eval` or external runtime origins.

Vercel is the selected Git-backed static host at `https://tylerwilhite.dev`, with the project namespace at `/workbench/`. Version 1.0.8 is the verified predecessor: audited source `fc2957843077606a1cdb8fe9101cbed9421fb243` contains no release summary; direct child `1c1c06b8e5c6973604b025b63aafed606b2bd522` adds only that generated file, annotated tag `v1.0.8` points to the child, and the same child was deployed. Version 1.0.9 begins a fresh audited-source lineage and cannot inherit that evidence. The hosted [release summary](https://tylerwilhite.dev/security/release-summary.json) and [deployment binding](https://tylerwilhite.dev/security/deployment-binding.json) are authoritative for whichever exact artifact is currently deployed. Preview builds still omit canonical-dependent output. Production builds accept the origin only through validated `SITE_CANONICAL_URL`, which drives HTML metadata, robots/sitemap output, and RFC 9116 canonical identity. This verifies an exact static release artifact, not Production reliability, shared identity/state, real external integrations, or live E2B/LiteLLM/provider operation. See [ADR: static hosting](docs/adr/static-hosting-and-security-headers.md) and [deployment verification](docs/deployment-verification.md).
