...
(function () {
  ...
  function genTimes(issue, id, status, i) {
    ...
    return { p: id + "@2.x", s: status === "none" ? "—" : "Jun 10 · " + base, c: (status === "done" || status === "fail") ? "Jun 10 · " + base : "—" };
  }
...
> Synthetic risk review artifact.`,
  ...
  "note": "Provenance is recorded so every artifact is traceable to a prompt version and input hash."
    ...
    "evidence.md": {
      type: "Markdown", lang: "md", pending: true,
...
> Synthetic evidence — produced by the simulated Verify run.`,
  ...
  const REVIEW_STATUS = {
    "inputs.json": "Approved", "intake.md": "Approved", "spec.md": "Approved",
    "plan.md": "Approved", "change-targets.json": "Review required",
    "risk-review.md": "Approved", "prompt-provenance.json": "Auto-recorded",
    "evidence.md": "Pending",
  };
...
> Clean-room synthetic content — not a real artifact.`;
  ...
  D.evidenceArtifact = function (issue) {
    ...
    return {
      ...
      name: "evidence.md", type: "Markdown", stage: "Verify", stageId: "verify",
      timestamp: "just now", reviewStatus: "Pending", risk: "Low", lang: "md",
  ...
  /* ---------- PR readiness ---------- */
  const PR_FILES_1150 = [
    { path: "frontend/src/app/variance-commentary/variance-commentary.component.ts",   category: "Angular UI",   add: 142, del: 0,  status: "expected" },
    { path: "frontend/src/app/variance-commentary/variance-commentary.component.html", category: "Angular UI",   add: 86,  del: 0,  status: "expected" },
    { path: "frontend/src/app/variance-commentary/variance-commentary.service.ts",     category: "Angular UI",   add: 64,  del: 0,  status: "expected" },
    { path: "backend/Finance.Api/Controllers/VarianceCommentaryController.cs",         category: "C#/.NET API",  add: 73,  del: 0,  status: "expected" },
    { path: "backend/Finance.Application/Services/VarianceCommentaryService.cs",        category: "C#/.NET API",  add: 118, del: 0,  status: "expected" },
    { path: "backend/Finance.Data/Repositories/VarianceRepository.cs",                  category: "C#/.NET API",  add: 22,  del: 6,  status: "expected" },
    { path: "database/oracle/views/VW_BUDGET_ACTUAL_VARIANCE.sql",                      category: "Oracle SQL",   add: 41,  del: 0,  status: "expected" },
    { path: "database/oracle/packages/PKG_VARIANCE_COMMENTARY.sql",                     category: "Oracle SQL",   add: 59,  del: 0,  status: "expected" },
    { path: "tests/Finance.Api.Tests/VarianceCommentaryTests.cs",                       category: "Tests",        add: 96,  del: 0,  status: "expected" },
    { path: "tests/Finance.Application.Tests/VarianceCommentaryServiceTests.cs",        category: "Tests",        add: 88,  del: 0,  status: "expected" },
    { path: "docs/evidence/FIN-1150-evidence.md",                                       category: "Documentation / evidence", add: 34, del: 0, status: "unexpected" },
  ];
  ...
  D.prFor = function (issue) {
    if (issue.key === "FIN-1150") {
      return {
        ...
        status: "Draft",
        branch: issue.branch, target: "main",
        author: "Jordan Vega", created: "Jun 10 · 10:04",
        commits: 6, files: PR_FILES_1150,
        ...
        checks: [
          ...
          { name: "Artifact review complete", status: "pending", detail: "change-targets.json awaiting review" },
    ...
    return {
      number: issue.pr, title: issue.key + ": " + issue.title, status: issue.prStatus,
      branch: issue.branch, target: "main", author: issue.assignee, created: issue.lastRun,
      commits: hasPR ? 4 : 0, files: hasPR ? PR_FILES_1150.slice(0, 6) : [],
      unexpected: 0, unresolvedComments: issue.flags.needsReview ? 1 : 0,
      summary: hasPR
        ? "Synthetic PR summary for " + issue.key + ". Changed files are grounded in the issue's change-targets artifact."
        : "No pull request has been created for this issue yet. Run the workflow through Implement, then create a mock PR.",
      checks: hasPR ? [
        { name: "Unit tests", status: issue.flags.failedVerification ? "fail" : "pass", detail: issue.flags.failedVerification ? "2 failed" : "passed" },
        { name: "No secrets detected", status: "pass", detail: "0 findings" },
        { name: "Human review required", status: "required", detail: "reviewer gate" },
      ] : [],
      reviewers: hasPR ? [{ name: issue.reviewer, role: "Reviewer", state: issue.flags.needsReview ? "pending" : "pending" }] : [],
      checklist: [
  ...
  D.validationFor = function (issue) {
    if (issue.key === "FIN-1150") {
      return {
        decision: "Pending", branch: issue.branch, commitSha: "a1c9f3e",
        evidenceStatus: "In Progress",
        acceptance: [
          { id: "AC1", text: "Analyst can request a draft commentary for an account + period", status: "Passed" },
          { id: "AC2", text: "Draft shows variance amount, % variance, and direction", status: "Passed" },
          { id: "AC3", text: "Draft labeled 'AI-generated — review required' until approved", status: "Passed" },
          { id: "AC4", text: "Analyst can edit the draft; edits are preserved", status: "In Progress" },
          { id: "AC5", text: "No commentary published without explicit human approval", status: "Passed" },
          { id: "AC6", text: "Inputs sourced from VW_BUDGET_ACTUAL_VARIANCE (read-only)", status: "Passed" },
          { id: "AC7", text: "Materiality threshold configurable per report", status: "Not Started" },
        ],
        scenarios: [
          { name: "VC-01 · Draft for material unfavorable variance", status: "Passed", note: "−$182k / −12.4%" },
          { name: "VC-02 · Draft for immaterial variance (below threshold)", status: "Passed", note: "suppressed as expected" },
          { name: "VC-03 · $0 budget line guard", status: "Passed", note: "no divide-by-zero" },
          { name: "VC-04 · Edit then approve preserves edits", status: "In Progress", note: "edit persistence under test" },
          { name: "VC-05 · Approval gate blocks publish", status: "Passed", note: "UI + API both block" },
        ],
        oracleAssumptions: [
        ...
        security: "Passed",
        testerNotes: [
          { author: "P. Shah", time: "Jun 10 · 11:20", text: "Approval gate solid in both paths. Verified publish is blocked without explicit approve." },
          { author: "P. Shah", time: "Jun 10 · 11:34", text: "Edit-persistence (AC4 / VC-04) still under test — re-checking buffer on re-open." },
        ],
      };
    }
    // generic
    ...
    return {
      ...
      branch: issue.branch, commitSha: "—", evidenceStatus: failed ? "Failed" : "Not Started",
      acceptance: [
        { id: "AC1", text: "Primary behavior for " + issue.title, status: failed ? "Failed" : "Not Started" },
      ...
      a11y: "Not Started", security: "Not Started",
      testerNotes: failed
        ? [{ author: issue.tester !== "—" ? issue.tester : "QA", time: issue.lastRun, text: "Verify failed — fixes requested before re-run." }]
        : [],
      empty: !failed,
    };
  };
