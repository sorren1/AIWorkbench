...
The direct evidence child deliberately renders the generated release summary that is absent from its audited parent, so its case-study HTML cannot equal the parent's recorded byte measurement. That one case still enforces every byte/request budget and is accepted only after the release-evidence validator proves that the current commit changes only `public/security/release-summary.json` and binds the hosted CodeQL result to its direct parent. This lets the evidence child complete its pull-request checks before the immutable release tag is created; `security:release-evidence:require` still requires that annotated tag before deployment. Any executable, CSS, source, or unrelated summary change continues to fail closed.
...
The portfolio index, case study, article, and 404 remain authored static HTML. React is emitted only for the demo entry, so opening either the personal-domain home page or the case study does not download the workbench bundle. No syntax-highlighting dependency exists; code excerpts use static HTML and CSS, leaving no heavy highlighting code to defer.
