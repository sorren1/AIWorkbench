...
- Intended evidence release: `v1.0.5` (`v1.0.3` and `v1.0.4` were abandoned before tagging when exact-main audits exposed separate release-blocking defects)
...
| Canonical screenshot determinism              | FIX IN REVIEW               | The v1.0.4 evidence audit alternated between two visually identical Architecture rasters. The alternate changed 27 isolated SVG-edge pixels with maximum channel delta 37 on two of three Linux runs. The comparison now uses a tested bounded antialias tolerance while still rejecting larger regions, stronger deltas, and dimension changes. |
...
| Exact tagged Preview                          | PENDING                     | Preview-scoped release bindings must be updated to the final `v1.0.5` tag, audited source commit, and evidence commit. A new Git branch at the exact tagged evidence commit will then trigger and prove an automatic Ready Preview.                                                                                                              |
...
4. Create and push annotated tag `v1.0.5` on the exact evidence child. Run `npm run security:release-evidence:require` against the tag.
