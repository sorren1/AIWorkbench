...
The first run may download the immutable Gitleaks and Trivy scanner images, Trivy's current vulnerability database, and the digest-pinned Node base. Scanner images, versions, base digest, thresholds, and target image are declared in `security/tooling.json`.
...
| `scanner-metadata.json`   | Tool versions, immutable scanner images, Docker version, target image ID and base digest |
...
- npm and container high/critical findings fail, including findings already present; this is stricter than only rejecting new findings.
...
The scanner report retains the original finding. The orchestrator matches the documented entry before applying policy, records the active suppression, and fails on malformed, duplicate, expired, or path-mismatched entries. No suppressions exist in this revision.
