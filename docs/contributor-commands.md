...
| `npm run demo:sandbox`          | Start a gitignored local run that pauses for bounded-write approval. |
...
The sandbox commands use `.workbench/runs/`, which is gitignored. See `docs/human-approval-protocol.md` for exact arguments and trust boundaries. They never operate on an external repository or make a network call.
