...
The Docker sandbox command accepts no repository, patch, or command input. It copies only `examples/toy-repo`, automatically prepares the exact Node image when needed, disables container networking, applies resource limits, and emits checked-in synthetic/public evidence. See `docs/vertical-slice-walkthrough.md` and `docs/sandbox-security-model.md`.
