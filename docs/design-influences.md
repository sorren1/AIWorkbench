...
- **Deliberately not adopted:** LiteLLM source code, SDK internals, example code, UI, copy, database schema changes, broad administrative dashboard, hosted service, automatic fallback policy, production high-availability topology, or a browser-facing proxy. The local derivative changes only the runtime package floor, Prisma cache ownership/location, and non-root execution needed for the digest-pinned database image; the proxy application remains upstream LiteLLM behind a loopback-only local profile.
