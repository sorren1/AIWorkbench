  ...
  return (
    <div className="wb-page">
      ...
      <div className="wb-mt-12">
        <Banner tone="info" title="Deterministic run · no model call" icon="sparkles">
          This recorded run has an <code>agent.invoke</code> span but no <code>model.call</code>{" "}
          span because the patch fixture is deterministic and no model was invoked. Model calls,
          tokens, and cost are exact zero—not estimated activity.
        </Banner>
      </div>
      ...
      <div className="wb-grid wb-grid-2 wb-mt-16">
        ...
        <Card>
          ...
          <dl className="wb-kv wb-trace-bindings">
            ...
            <dt>Source commit</dt>
            <dd>
              <code>{evidence.sourceCommit}</code> | {evidence.sourceWorkingTree.toLowerCase()}
            </dd>
            <dt>Tested tree</dt>
            <dd>
      ...
      <Card className="wb-mt-16">
        <CardHead
          icon="file-text"
          title="Recorded evidence bundle"
          sub={`${evidence.sandboxProvider} · ${evidence.runId}`}
          actions={<Badge tone="neutral">Checked-in validated snapshot</Badge>}
        />
        <div className="wb-card-body wb-stack-sm">
          <p className="wb-secondary wb-text-sm">
            Generated by an explicitly invoked local developer command over the synthetic toy
            repository. These files are recordings; opening them does not execute code.
          </p>
          <div className="wb-flex wb-wrap wb-gap-8">
            <a
              className="wb-btn wb-btn--secondary wb-btn--sm"
              href={`../recorded-evidence/${evidence.evidenceArtifact}`}
            >
              Evidence JSON
            </a>
            <a
              className="wb-btn wb-btn--secondary wb-btn--sm"
              href={`../recorded-evidence/${evidence.evidenceMarkdownArtifact}`}
            >
              Evidence Markdown
            </a>
            <a
              className="wb-btn wb-btn--secondary wb-btn--sm"
              href={`../recorded-evidence/${evidence.traceArtifact}`}
            >
              Trace JSON
            </a>
          </div>
