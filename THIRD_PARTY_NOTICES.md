...
| MIT        | `@eslint/js`, `@testing-library/jest-dom`, `@testing-library/react`, `@types/node`, `@types/pngjs`, `@types/react`, `@types/react-dom`, `@vitejs/plugin-react`, `@vitest/coverage-v8`, `eslint`, `eslint-plugin-react-hooks`, `globals`, `jsdom`, `pngjs`, `prettier`, `tsx`, `typescript-eslint`, `vite`, `vitest` |
...
| `public/assets/social-card.svg` and generated `.png`   | Original first-party vector composed from local project marks, type, and shapes; no stock illustration.                                                       |
| `public/assets/screenshots/*.png`                      | Reproducibly captured by `npm run screenshots:generate` from the first-party built application; no manually composited or external screenshot material.       |
